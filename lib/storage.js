...
const DEFAULT_BANK_IMPORTS = ['bank-soal-rpl-online.md', 'bank-soal-pbo-java.md'];
const EXAM_SELECT_BASE = 'id, slug, title, description, duration_minutes, shuffle_questions, shuffle_options, published, bank_id';
const EXAM_SELECT_WITH_COURSE = 'id, slug, title, course_name, description, duration_minutes, shuffle_questions, shuffle_options, published, bank_id';

function getCourseName(bank) {
  return (bank.course ?? bank.title ?? '').trim();
}

function getImportSlug(sourcePath, courseName, fallbackTitle) {
  const sourceSlug = slugify(path.basename(sourcePath, path.extname(sourcePath)));
  return sourceSlug || slugify(courseName || fallbackTitle);
}

async function selectExamById(supabase, examId) {
  const withCourse = await supabase
    .from('exams')
    .select(EXAM_SELECT_WITH_COURSE)
    .eq('id', examId)
    .maybeSingle();
...
async function listPublishedExams(supabase) {
  const withCourse = await supabase
    .from('exams')
    .select(EXAM_SELECT_WITH_COURSE)
    .eq('published', true)
...
async function getQuestionsFromSupabase(supabase, examId) {
  const { data: exam, error: examError } = await selectExamById(supabase, examId);
...
async function listExamsFromSupabase(supabase) {
  const { data, error } = await listPublishedExams(supabase);
...
export async function getSummary() {
  ...
  return {
    bankCount: summary.bankCount ?? 0,
    questionCount: summary.questionCount,
    examCount: summary.examCount ?? 1,
...
export async function importMarkdownBankToSupabase(sourcePath = 'bank-soal-rpl-online.md') {
  if (!hasSupabaseConfig()) {
    throw new Error('Supabase configuration is missing.');
  }

  const supabase = createSupabaseAdminClient();
  const bank = await parseMarkdownBank(path.join(process.cwd(), sourcePath));
  const courseName = getCourseName(bank);
  const bankSlug = getImportSlug(sourcePath, courseName, bank.title);
  const examSlug = bankSlug;
  ...
  if (existingBank?.id) {
    ...
    const result = await supabase.from('question_banks').insert(bankPayload).select('*').single();
    bankRow = result.data;
    bankError = result.error;
  }
  ...
  const examPayload = {
    ...
    course_name: courseName,
    description: `Ujian online ${courseName || bank.title} berbasis bank soal Markdown`,
    duration_minutes: 60,
  ...
  const { data: existingExam } = await supabase
    .from('exams')
    .select('id')
    .eq('bank_id', bankRow.id)
    .maybeSingle();

  let examRow;
  let examError;
  if (existingExam?.id) {
    const result = await supabase.from('exams').update(examPayload).eq('id', existingExam.id).select('*').single();
    examRow = result.data;
    examError = result.error;
  } else {
    const result = await supabase.from('exams').insert(examPayload).select('*').single();
    examRow = result.data;
    examError = result.error;
  }
...
export async function importMarkdownBanksToSupabase(sourcePaths = DEFAULT_BANK_IMPORTS) {
  const results = [];
  for (const sourcePath of sourcePaths) {
    results.push(await importMarkdownBankToSupabase(sourcePath));
  }

  return {
    importedCount: results.length,
    questionCount: results.reduce((total, item) => total + (item.questionCount ?? 0), 0),
    results
  };
}
...
async function getGuestAttemptBundleFromSupabase(supabase, attemptId) {
  ...
  const { data: exam, error: examError } = await selectExamById(supabase, attempt.exam_id);
