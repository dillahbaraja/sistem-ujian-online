...
import { parseMarkdownBankSync } from './markdown.js';
...
function createInitialStateSync() {
  const banks = DEMO_BANK_FILES.map((filePath) => parseMarkdownBankSync(path.join(process.cwd(), filePath)));
...
export function getMockStore() {
  if (!globalThis.__UJIAN_ONLINE_MOCK__) {
    globalThis.__UJIAN_ONLINE_MOCK__ = createInitialStateSync();
  }
  return globalThis.__UJIAN_ONLINE_MOCK__;
}
